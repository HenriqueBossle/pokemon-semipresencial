...
<form class="max-w-sm mx-auto" action="{{ url('pokemon/'.$pokemon->id) }}" method="POST" enctype="multipart/form-data">
    ...
    @method('PUT')
        ...
        <div>
            <img class="w-24 h-24 rounded-full shadow-lg" src="{{ asset($pokemon->image) }}" alt="{{ $pokemon->name }}">
        ...
        <button type="submit" class="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Update Pokemon</button>
    </form>
@endsection