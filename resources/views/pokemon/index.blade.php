...
<div class="flex flex-wrap justify-center mt-10">
    ...
    @foreach($pokemon as $entity)
        <div class="p-4 max-w-sm">
            <div class="flex rounded-lg h-full dark:bg-gray-800 bg-slate-800 p-8 flex-col">
                ...
                <span class="text-sm font-medium text-white">{{ $entity->type }}</span>
                <span class="text-sm font-medium text-white">{{ $entity->power }}</span>
                @if (isset($entity->coach))
                   <span class=" text-sm text-white">Pertence a {{ $entity->coach->name }}</span>
                @else
